...
---
### Step 5: TF(좌표계) 이해 및 모델 구조 최적화 (2026-05-18)
- **수행 내용**: 바퀴 회전 시각화를 위한 인디케이터를 독립 링크로 분리하고, URDF의 기하학적 속성 및 재질 설정을 최적화함.
- **학습 포인트**:
    - **Box Size의 의미**: `<box size="X Y Z">`에서 각 값은 로컬 좌표계 기준 **앞뒤(x)**, **좌우(y)**, **높이(z)** 의 길이를 의미함.(단위: 미터)
    - **URDF 재질(Material) 전역 관리**: 재질 정의를 최상단으로 분리하여 이름 충동을 방지하고 코드의 재사용성을 높임.
    - **링크 분리(Link Separation)**: 하나의 링크 안에 여러 시각 요소가 있을 때 발생하는 재질 간섭(색상 상속) 문제를 해결하기 위해, 별도의 링크를 생성하고 fixed joint로 연결하는 설계 패턴을 학습함.
- **트러블슈팅 기록**:
